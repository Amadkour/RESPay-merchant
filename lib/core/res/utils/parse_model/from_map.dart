...
class FromMap {
  ...
  bool? convertToBool({required String key, bool? defaultValue, Map<String, dynamic>? innerMap}) {
    final Map<String, dynamic> usedMap = innerMap ?? map;
  ...
  List<T> convertToListOFModel<T extends ParentModel>({dynamic jsonData, dynamic modelInstance}) {
  ...
  List<DateTime> convertToDateTimeList({dynamic jsonData, DateFormat? dateFormat}) {
    final DateFormat usedDateFormat = dateFormat ?? DateFormat('dd-MM-yyyy');

    return jsonData is List<dynamic>
        ? jsonData.map((dynamic e) => usedDateFormat.parse(e.toString())).toList()
        : <DateTime>[];
  }
